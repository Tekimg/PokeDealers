...
class Transaction(object):
    ...
    def create(cls, buy_order: str, session_id: str, amount: float, return_url: str, options: Options = None) \
        ...
        try:
            ...
            # Imprimir la respuesta de la API antes de procesarla
            print(f"Status Code: {response.status_code}")
            print(f"Response: {response.text}")  # Esta línea está correcta para la respuesta HTTP
            ...
            dict_response = TransactionCreateResponseSchema().loads(response.text).data
            print(f"Respuesta deserializada: {dict_response}")  # Para diagnóstico

            # Extraer los valores del diccionario y crear el objeto TransactionCreateResponse
            transaction_response = TransactionCreateResponse(
                token=dict_response['token'],  # Extraer el valor de 'token'
                url=dict_response['url']       # Extraer el valor de 'url'
            )

            # Ahora imprime los datos directamente desde el objeto TransactionCreateResponse
            print(f"Token: {transaction_response.token}")
            print(f"URL: {transaction_response.url}")
            ...
            print("Código:", e.code)
            raise  # Volver a lanzar el error después de manejarlo
